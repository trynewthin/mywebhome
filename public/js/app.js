...
function applyTheme() {
  ...
  // 风格属性
  const style = localStorage.getItem('mywebhome-style') || 'brutalist';
...
function renderSettings() {
  ...
  const currentStyle = localStorage.getItem('mywebhome-style') || 'brutalist';
        ...
        <div class="settings-card" data-settings-title="外观">
          ...
          <div class="settings-row">
            <div class="settings-row-info">
              ...
              <div class="settings-row-desc">切换完全不同的视觉设计语言</div>
            </div>
            <select class="form-select" id="setting-style" style="width: auto; min-width: 140px;">
              <option value="brutalist" ${currentStyle === 'brutalist' ? 'selected' : ''}>🔩 工业终端</option>
              <option value="zen" ${currentStyle === 'zen' ? 'selected' : ''}>🍵 和式极简</option>
              <option value="aurora" ${currentStyle === 'aurora' ? 'selected' : ''}>🌌 极光玻璃</option>
            </select>
          </div>
  ...
  document.getElementById('setting-style').addEventListener('change', (e) => {
    localStorage.setItem('mywebhome-style', e.target.value);
    applyTheme();
  });
